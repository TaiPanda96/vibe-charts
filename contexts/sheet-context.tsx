...
import { ChartConfig, QuestionStep } from "../types/chart";
...
interface SheetContextType {
  ...
  updateSheetWizardState: (
    id: string,
    state: Partial<SheetWizardState>,
...
export function SheetProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (activeSheet?.wizardState.wizardStep) {
      ...
      const targetPath = getPathForWizardStep(
        activeSheet.wizardState.wizardStep
      );
  ...
  const getPathForWizardStep = (
    step: "landing" | "questions" | "result"
  ): string => {
    switch (step) {
      case "landing":
        return "/";
