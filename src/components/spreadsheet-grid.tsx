...
export function SpreadsheetGrid({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Enter") {
      ...
      if (colIndex < cols - 1) {
        const nextCell = `${String.fromCharCode(65 + colIndex + 1)}${
          rowIndex + 1
        }`;
        setSelectedCell(nextCell);
        setEditValue(cellData[nextCell]?.value || "");
  ...
  const extraRowNumbers =
    extraRows > 0
      ? Array.from({ length: extraRows }, (_, i) => rows + i + 1)
      : [];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center">
        <div className="pointer-events-auto content-container no-scroll">
          {children}
        </div>
      </div>
                  ...
                  return (
                    <td
                      ...
                      className={`w-[100px] h-6 border-r border-b border-[#e0e0e0] ${
                        isSelected
                          ? "bg-[#e8f0fe] outline outline-1 outline-[#000080]"
                          : "bg-white"
                      }`}
                    ...
                    >
                      {isSelected && isEditing ? (
                        ...
                        <div className="px-1 text-xs truncate w-full">
